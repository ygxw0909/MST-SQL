...
class HydraEvaluator():
    ...
    def _eval_imp(self, eval_data: SQLDataset, get_sq=True):
        ...
        for input_feature, model_output in zip(eval_data.input_features, model_outputs):
            ...
            agg, select, where, conditions, _ = self.model.parse_output(input_feature, model_output, wc_label)
            if agg != agg_label:
                cur_acc["agg"] = 0
        ...
        return result_str, sq, acc["overall"] * 100.0 / cnt

    def eval(self, epochs):
        print(self.bad_case_dir)
        overall_acc = 0.0
        for eval_file in self.eval_data:
            result_str, sq, acc = self._eval_imp(self.eval_data[eval_file])
            overall_acc += acc
        ...
        return float(overall_acc / len(self.eval_data.keys()))